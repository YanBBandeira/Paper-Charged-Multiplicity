...
# Mapeamento das 6 configurações de simulação
casos = {
...
particulas_info = {
    ...
    "D_mesons": {
        "sigla": "d",
        "label": r"$D$ Mesons ($D^\pm, D_s^\pm$)",
        "xlabel": r"$N_{D}$",
    },
    "Kaons": {
        "sigla": "k",
        "label": r"Kaons ($K^{\pm}, K_S^0, K_L^0, K^0/\bar{K}^0$)",
        "xlabel": r"$N_{K}$",
    },
}
...
log_file = open("validacao_simulacao.log", "w", encoding="utf-8")
...
def processar_e_validar_dados(arq_mult, arq_eta):
    ...
    N_valores = dados_mult[:, 0]  # Eixo X (Já centralizado no zero no Pythia)
...
def salvar_grafico(dados_plot, xlabel, ylabel, label, y_min, nome_pdf, y_log=True, leg_title="", subpasta=""):
    ...
    if max_x_global > 0:
        plt.xlim(left=0, right=max_x_global * 1.05)
...
for corte_eta in cortes_eta:
    ...
    for p_nome, p_conf in particulas_info.items():
        ...
        dados_pn_soft, dados_evt_soft, dados_dndeta_soft = [], [], []
        dados_pn_hard, dados_evt_hard, dados_dndeta_hard = [], [], []
        ...
        for idx, (nome_caso, (pref_soft, pref_hard)) in enumerate(casos.items()):
            ...
            if res_s is not None:
                linha_s = f"{nome_caso:<15} | {'Soft':<6} | {res_s['soma_prob']:<10.4f} | {res_s['media_N']:<8.2f} | {res_s['integral']:<10.2f} | {status_s}"
                log_print(linha_s)
                
                # Criação clara e legível linha por linha dos dicionários
                item_pn_s = {
                    "x": res_s["N"], 
                    "y": res_s["P_N"], 
                ...
                dados_pn_soft.append(item_pn_s)
                ...
                dados_evt_soft.append(item_evt_s)
                ...
                dados_dndeta_soft.append(item_eta_s)
            ...
            if res_h is not None:
                linha_h = f"{nome_caso:<15} | {'Hard':<6} | {res_h['soma_prob']:<10.4f} | {res_h['media_N']:<8.2f} | {res_h['integral']:<10.2f} | {status_h}"
                log_print(linha_h)
                ...
                dados_pn_hard.append(item_pn_h)
                ...
                dados_evt_hard.append(item_evt_h)
                ...
                dados_dndeta_hard.append(item_eta_h)

        # =====================================================================
        # 4.3. GERAÇÃO DOS PLOTS COMPARATIVOS
        # =====================================================================
        if dados_pn_soft:
            ...
            salvar_grafico(dados_dndeta_soft, r"Pseudorapidity ($\eta$)", r"$dN/d\eta$", p_conf["label"], 0, 
        ...
        if dados_pn_hard:
            ...
            salvar_grafico(dados_dndeta_hard, r"Pseudorapidity ($\eta$)", r"$dN/d\eta$", p_conf["label"], 0, 
                           f"{p_nome}_dNdeta_eta_{corte_str}.pdf", y_log=False, leg_title=rf"$|\eta| < {corte_eta}$ (Hard)", subpasta=sub_dir_hard)

msg_fim = f"\n{'='*85}\nProcessamento, testes de validação e geração dos plots comparativos concluídos!\nDetalhes salvos em 'validacao_simulacao.log'\n{'='*85}"
log_print(msg_fim)
